...
#Packages used: "readxl", "shiny", "ggplot2" 
  ...
  server <- function(input, output) {
    ...
    output$myplot <- shiny::renderPlot({
      ...
      if (answer == "2022 election party vote counts"){
        ggplot2::ggplot(Sweden_election$election_data,aes(Parti,sort(Röster_2022,decreasing = TRUE)))+geom_col(fill = "forestgreen")+
        ...
        total<-sum(Sweden_election$election_data$Röster_2022)
        Sweden_election$election_data$Party<-Sweden_election$election_data$Parti
        Sweden_election$election_data$share<-(Sweden_election$election_data$Röster_2022/total)*100
